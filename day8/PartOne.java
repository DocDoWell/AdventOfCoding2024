...
public class PartOne {
    ...
    /*public static void main(String[] args) {
        try (Stream<String> stream = Files.lines(Paths.get("day8/input.txt"))) {
            ...
            System.out.println("Part One: "+ uniqueLocations.size());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        for(int i = 0; i < pairs.size()-1; i++){
            ...
            for(int j = i+1; j < pairs.size(); j++){
                ...
                if(isAntiNodeOnGrid(firstPair,secondPair,horizontalBoundary,verticalBoundary)){
                    uniqueLocations.add(new Pair(firstPair.row + (firstPair.row - secondPair.row),firstPair.col +(firstPair.col - secondPair.col)));
                }
                if(isAntiNodeOnGrid(secondPair,firstPair,horizontalBoundary,verticalBoundary)){
                    uniqueLocations.add(new Pair(secondPair.row + (secondPair.row - firstPair.row),secondPair.col +(secondPair.col - firstPair.col)));
                }
            }
        }
